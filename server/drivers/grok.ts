...
// generateText (bot titles, thread names) — t3code's TextGeneration slot.
import type {
  DriverCreateInput,
  ProviderDriver,
  ProviderInstance,
  ...
  async create(input: DriverCreateInput<GrokConfig>): Promise<ProviderInstance> {
    ...
    const snapshot = async (): Promise<ProviderSnapshot> => {
      if (!apiKey) {
        return {
          state: "unavailable",
          reason: `no xAI API key — add {"xai":{"key":"xai-…"}} to ~/.opengrokbot/config.json or set ${config.apiKeyEnv}`,
        };
      }
