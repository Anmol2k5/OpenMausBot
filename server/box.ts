...
export async function provisionBox(cfg: AppConfig, botId: string, botName: string) {
  if (!boxConfigured(cfg)) {
    throw new Error('box provider not enabled — add {"box":{"token":"…"}} to ~/.opengrokbot/config.json');
  }
  ...
  const bootstrap = [
    ...
    `tmux has-session -t work 2>/dev/null || tmux new-session -d -s work 'echo; echo "  ▦ ${botName.replace(/["'\\\\]/g, "")}'"'"'s computer — OpenGrokBot"; echo; exec bash -i'`,
    "echo bootstrapped",
  ].join("\n");
  let boot;
